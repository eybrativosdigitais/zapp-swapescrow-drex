...
contract MerkleTree is MiMC {
    ...
    */
     ...
     */
    event NewLeaf(uint256 leafIndex, uint256 leafValue, uint256 root);
    event NewLeaves(uint256 minLeafIndex, uint256[] leafValues, uint256 root);

    // event Output(uint[] input, uint[] output, uint prevNodeIndex, uint nodeIndex); // for debugging only

    uint256 public zero = 0;
    uint256 public treeHeight = 32; //
    uint256 public treeWidth = 2 ** treeHeight;
     ...
     */
    uint256[33] public frontier; //  = new uint[](treeHeight + 1); // the right-most 'frontier' of nodes required to calculate the new root when the next new leaf value is added.
     ...
     */
    function insertLeaf(uint256 leafValue) public returns (uint256 root) {
        ...
        uint256 nodeValue = leafValue; // nodeValue is the hash, which iteratively gets overridden to the top of the tree until it becomes the root.

        uint256[] memory input = new uint256[](2); //input of the hash fuction
        uint256[] memory output = new uint256[](1); // output of the hash function
     ...
     */
    function insertLeaves(uint256[] memory leafValues) public returns (uint256 root) {
        ...
        uint256[] memory output = new uint256[](1); // the output of the hash
    ...
    uint256 public constant Initial_NullifierRoot =
