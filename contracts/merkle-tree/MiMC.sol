...
contract MiMC {
    ...
    function Encipher(uint256 in_x, uint256 in_k) public pure returns (uint256 out_x) {
    ...
    function Hash(uint256[] memory in_msgs, uint256 in_key) public pure returns (uint256) {
        bytes4 seed = 0x6d696d63; //this is 'mimc' in hex
        return MiMCp_mp(in_msgs, in_key, uint256(keccak256(abi.encodePacked(seed))), 91);
    }

    function mimcHash(uint256[] memory in_msgs) public pure returns (uint256) {
