...
import { getContractAddress, getContractInstance } from './common/contract.mjs'
...
export async function service_balanceOfToken(req, res, next) {
	try {
		let { token, owner = process.env.DEFAULT_ACCOUNT } = { ...req.body, ...req.query };
		...
		if (!web3Instance.utils.isAddress(token)) {
			errors.push({
				path: 'token',
				message: `Invalid address, received: ${token}`
			});
		}
		...
		// Todo: Fix using proper web3js formatter
		const balanceFormatted = balance / 10 ** decimals.toString()
...
export async function service_shieldedBalance(req, res, next) {
	try {
		...
		if (!Array.isArray(erc20Tokens)) {
			errors.push({
				path: 'erc20Tokens',
				message: `Invalid array, received: ${erc20Tokens}`
			});
		} else {
			erc20Tokens.forEach((token, index) => {
				if (!web3Instance.utils.isAddress(token)) {
					errors.push({
						path: `erc20Tokens[${index}]`,
		...
		const extractBalances = parseCommitments(owner, erc20Tokens, erc1155TokenIds);
		const commitments = await getAllCommitments();
...
export async function service_stats(req, res, next) {
	try {
		...
		console.log('SwapShield', SwapShield)
		const [
			swapIdCounter,
			latestRoot,
			ownPublicKey,
			...
			ERC20.methods.allowance(process.env.DEFAULT_ACCOUNT, SwapShield._address).call(),
			ERC1155Token.methods.isApprovedForAll(process.env.DEFAULT_ACCOUNT, SwapShield._address).call(),
		]);
