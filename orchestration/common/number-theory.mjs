...
function decompressStarlightKey (publicKeyInt) {
  ...
  if (!isOnCurve([x.bigInt, y.bigInt])) { throw new Error('The computed point was not on the Babyjubjub curve') }
  return [x, y]
}
...
function sharedSecretKey (secretKey, recipientPublicKey) {
  ...
  const sharePublicKeyPoint = generalise(
  ...
  const sharedPublicKey = new GN(sign + yBits.padStart(253, '0'), 'binary')
